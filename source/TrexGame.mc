...
class TrexGame extends WatchUi.InputDelegate {
    ...
    hidden function drawCurrentScore(dc) {
        dc.drawText(screenWidth - 5, 2, Graphics.FONT_SMALL, formatScoreNumber(score), Graphics.TEXT_JUSTIFY_RIGHT);
    }

    hidden function drawHighScore(dc) {
        dc.drawText(5, 2, Graphics.FONT_SMALL, "HI " + formatScoreNumber(highScore), Graphics.TEXT_JUSTIFY_LEFT);
    }
